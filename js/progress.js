(function($){
    var functions = {
        init:function(input, step, max, min, digit){
            ...
            input.css("width", width-height*2-50);
            ...
            //添加左右加减号
            input.wrap("<div style = 'overflow:hidden;width:" + (width+5) + "px;height:" + height + "px;border: 1px solid #CCC;'></div>");
            ...
            input.after("<div style='text-align:center;width:" + (height+21.5) + "px;height:" + height + "px;line-height:" + height + "px;float: left;position: relative;'>&nbsp;Mbps</div><div id = '" + input.attr('id') + "r'  onselectstart = 'return false;' style = '-moz-user-select:none;cursor:pointer;text-align:center;width:" + height + "px;height:" + height + "px;line-height:" + height + "px;border-left: 1px solid #CCC;float:right'> + </div>");
            //左右调用执行
            $("#" + input.attr('id') + "l").click(function(){
                _this.execute(input, step, max, min, digit, true);
                //通过值来计算出边距和宽度
                var inputval =$("#progressval").val();
                mask.style.width =(inputval/200)*(scroll.offsetWidth-bar.offsetWidth)+"px";
                bar.style.left = (inputval/200)*(scroll.offsetWidth-bar.offsetWidth)+"px";
    ...
    $(function(){
        ...
        inputs.each(function(){
            //预设值数据选择
            var data = {
                default_data : 	{"step" : 1, "min" : 0, "max" : 200, "digit" : 0},
...
var scroll = document.getElementById('barscroll');
var bar = document.getElementById('barbtn');
var mask = document.getElementById('masktools');
...
bar.onmousedown = function(event){
    ...
    document.onmousemove = function(event){
        ...
        // jquery设置值
        $("#progressval").val(parseInt(barleft/(scroll.offsetWidth-bar.offsetWidth) * 200));
